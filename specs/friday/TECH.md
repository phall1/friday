...
- Persistence schema 2 retains only onboarding completion, hotkey confirmation, selected model, and double-tap/overlay/delivery/login preferences. A central pure durable projection replaces live workflow with `booting`, clears session/generation/source/result/error bytes, and removes ambient permission/model/readiness facts before every `Cmd.persist`; restore applies the same scrub before rechecking native facts. The only schema-1 data came from unreleased spike builds; its pure migration resets to defaults. A signed relaunch reloaded the active model and exited cleanly, while core contract tests prove source tokens, transcripts, failures, and ambient facts cannot enter the persisted model.
