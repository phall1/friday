...
- A long fixture was canceled through the keyed host request; no late transcript replaced the canceled state after 10 seconds, and a fresh generation transcribed successfully afterward. Explicit NeMo unload followed by packaged-app termination exited cleanly, and the final `NSApplicationWillTerminate` unload fence also let a warm packaged app terminate with exit 0 without a manual unload action.
...
- Phase 4 replaces the callback/count-only bridge with a per-key in-flight table and exactly-once native completion registry. Cancel synchronously completes the native key, late callbacks are ignored, teardown marks the host closing, clears channel/service handles under the spin mutex, cancels every native key, and frees the Zig callback context only after the in-flight table reaches zero; otherwise it deliberately retains the context rather than risking use-after-free.
- Hardware capture now uses `AVAudioSinkNode`; its receiver block touches only a precomputed C++ realtime state, atomics, raw `AudioBufferList`, and the SPSC ring. `AVAudioConverter` performs maximum-quality sample-rate conversion on the serial worker queue, including end-of-stream flush and deterministic output-frame accounting. Launch sweeps Friday audio temp; success/silence/delivery/dismiss/new-session/exit discard audio; only transcription failure retains retry audio. The packaged silence smoke left no file under `TemporaryItems/Friday/Audio`.
- `ModelRepository` now has one queue for every public mutation and URLSession delegate callback. `resume.json` binds URL/revision/artifact/expected bytes/partial size plus ETag or Last-Modified, and 206 responses must match `Content-Range` start and total. Model, manifest, staging/final/model-root, and index publication use fsync plus rename without deleting an active final first. Managed deletion resolves symlinks and refuses paths outside Friday’s model root; local models require a complete integrity/license/language/engine/family sidecar before runtime probing.
...
- Persistence schema 2 stores onboarding completion, hotkey confirmation, selected model, double-tap/overlay/delivery/login preferences, and the union model. Ambient permission/model facts are reset and rechecked after restore. The only schema-1 data came from unreleased spike builds; its pure migration resets to defaults. A signed relaunch restored onboarding/hotkey/model readiness while the immediate no-speech result disappeared, proving no transcript history persistence.
- The final signed production smoke exercised menu Start → exact-source capture → AVAudioEngine capture → stop/drain → warm local NeMo → silence result → ready, then confirmed temp audio deletion. The harness could not acoustically route the synthesized fixture into the physical microphone, so the packaged mic smoke reached the real ASR silence contract rather than delivery fallback; warm fixture ASR and truthful TextEdit/Terminal clipboard fallback remain separately proven.
