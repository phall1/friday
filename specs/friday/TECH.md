...
- `ModelRepository` is pure Zig and serializes every public mutation and HTTP callback through one owner. `std.http`, Zig file APIs, SHA-256, and JSON implement the repository without Foundation or URLSession. `resume.json` binds URL/revision/artifact/expected bytes/partial size plus ETag or Last-Modified, and 206 responses must match `Content-Range` start and total. Model, manifest, staging/final/model-root, and index publication use fsync plus rename. An exact existing final is revalidated byte-for-byte and runtime-probed before reuse; an invalid collision is never deleted or trusted, and verified staging publishes under a fresh managed revision directory. Startup clears missing/corrupt active records, and readiness remains false until recognizer activation succeeds. Managed deletion resolves symlinks and refuses paths outside Friday’s model root; local models remain references only.
...
- Models projects only verified repository rows with active/available state, exact source/license/language/size metadata, select, reference-only removal, and managed-copy deletion. Local picker copy names the required matching manifest sidecar. Hugging Face resolution never trusts repository names/tags as family or compatibility proof: a public `owner/repository` may yield only an **unverified immutable candidate** after bounded metadata establishes a 40-hex revision, exactly one top-level GGUF, ASR/GGUF hints, LFS SHA-256/size, license, and attribution. The candidate is neither listed nor selectable. A second confirmation authorizes only those exact bytes for local verification. Publication requires exact integrity, bounded structural GGUF header inspection, and the real NeMo create/destroy runtime probe. Because the untrusted metadata and structural header cannot prove an ASR family safely, a successful Hugging Face candidate records `runtime_verified_asr`; runtime failure removes staging and never publishes.
