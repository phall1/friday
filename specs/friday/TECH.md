...
- `FRIDAY_AUTOMATION_SCENE` is an environment-only launch channel with no release control. Twelve deterministic 760×620 reference-renderer PNGs cover onboarding, settings, model management, transcription error, recording, transcribing, the overlay control contract, missing-Accessibility recovery, unsupported Intel hardware, shortcut conflict, resumable-download recovery, and resolved Hugging Face confirmation across light and dark appearances. The automation script preserves user window state, asserts accessible roles/names and keyboard focus traversal, compares captures by default, and updates only under explicit `--update`; every committed golden comparison passed without runtime dispatch errors.
