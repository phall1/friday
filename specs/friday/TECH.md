...
- Model Manager projects every bounded repository row with active/available state, exact source/license/language/size metadata, select, reference-only removal, and managed-copy deletion. Local picker copy names the required matching manifest sidecar. The constrained Hugging Face path still accepts only the pinned immutable compatible source, and now requires an explicit hosting/network/~714 MB/CC-BY-4.0 confirmation before any request starts.
...
- `FRIDAY_AUTOMATION_SCENE` is an environment-only launch channel with no release control. Eight deterministic 760×620 reference-renderer PNGs cover onboarding, settings, model management, transcription error, recording, transcribing, the overlay control contract, and missing-Accessibility recovery across light and dark appearances. The automation script preserves user window state, asserts accessible roles/names and keyboard focus traversal, compares captures by default, and updates only under explicit `--update`; every committed golden comparison passed without runtime dispatch errors.
