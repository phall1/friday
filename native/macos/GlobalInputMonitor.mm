...
- (BOOL)configureFromString:(NSString *)configuration error:(NSError **)error {
    ...
    if ([values[@"command"] boolValue]) flags |= kCGEventFlagMaskCommand;
    if ([values[@"shift"] boolValue]) flags |= kCGEventFlagMaskShift;
    if ([values[@"option"] boolValue]) flags |= kCGEventFlagMaskAlternate;
    if ([values[@"control"] boolValue]) flags |= kCGEventFlagMaskControl;
    if ([values[@"fn"] boolValue]) flags |= kCGEventFlagMaskSecondaryFn;
    if (key < -1 || key > 127 || (key == -1 && flags == 0)) {
        if (error) *error = [NSError errorWithDomain:@"com.phall.friday.input" code:1 userInfo:@{NSLocalizedDescriptionKey:@"The shortcut is not globally distinguishable."}];
        return NO;
    }
    self.keyCode = key;
...
static CGEventRef FridayTap(CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *context) {
    (void)proxy;
    FridayGlobalInputMonitor *monitor = (__bridge FridayGlobalInputMonitor *)context;
    if (type == kCGEventTapDisabledByTimeout || type == kCGEventTapDisabledByUserInput) {
        if (monitor.tap) CGEventTapEnable(monitor.tap, true);
    } else {
...
- (BOOL)start:(NSError **)error {
    ...
    if (!self.inputMonitoringUsable) {
        if (error) *error = [NSError errorWithDomain:@"com.phall.friday.input" code:2 userInfo:@{NSLocalizedDescriptionKey:@"Input Monitoring permission is required for the global shortcut."}];
        return NO;
    }
    ...
    if (!self.tap) {
        if (error) *error = [NSError errorWithDomain:@"com.phall.friday.input" code:3 userInfo:@{NSLocalizedDescriptionKey:@"Friday could not create the global event monitor."}];
        return NO;
    }
...
- (void)stop {
    ...
    if (self.tap) { CFMachPortInvalidate(self.tap); CFRelease(self.tap); self.tap = NULL; }
    self.running = NO;
    self.chordDown = NO;
}

- (CGEventFlags)relevant:(CGEventFlags)flags {
    return flags & (kCGEventFlagMaskCommand | kCGEventFlagMaskShift | kCGEventFlagMaskAlternate | kCGEventFlagMaskControl | kCGEventFlagMaskSecondaryFn);
}
...
- (void)handleType:(CGEventType)type event:(CGEventRef)event {
    ...
    if (self.keyCode == -1) {
        ...
        if (down && !self.chordDown) [self emitDown:event];
        if (!down && self.chordDown) [self emitUp:event];
        self.chordDown = down;
        return;
    }
    NSInteger key = CGEventGetIntegerValueField(event, kCGKeyboardEventKeycode);
    if (key != self.keyCode) return;
    if (type == kCGEventKeyDown && flags == self.requiredFlags && CGEventGetIntegerValueField(event, kCGKeyboardEventAutorepeat) == 0) [self emitDown:event];
    if (type == kCGEventKeyUp) [self emitUp:event];
}
...
- (void)emitUp:(CGEventRef)event {
    self.probeUp += 1;
    self.handler(@"hotkey_up", @{@"atMs":@((double)CGEventGetTimestamp(event)/1e6)});
}

- (void)sleep:(NSNotification *)note { (void)note; self.chordDown = NO; }
...
- (NSDictionary<NSString *, id> *)runSyntheticProbe {
    NSUInteger beforeDown = self.probeDown;
    NSUInteger beforeUp = self.probeUp;
    NSError *error = nil;
    [self configureFromString:@"key=-1;command=1;shift=1;option=0;control=0;fn=0" error:&error];
    ...
    for (NSInteger tap = 0; tap < 2; ++tap) {
        CGEventRef down = CGEventCreateKeyboardEvent(NULL, 56, true);
        CGEventSetFlags(down, kCGEventFlagMaskCommand | kCGEventFlagMaskShift);
        CGEventPost(kCGHIDEventTap, down);
        CFRelease(down);
        CGEventRef up = CGEventCreateKeyboardEvent(NULL, 56, false);
        CGEventSetFlags(up, 0);
        CGEventPost(kCGHIDEventTap, up);
        CFRelease(up);
    }
    NSDate *deadline = [NSDate dateWithTimeIntervalSinceNow:1.0];
    while ([deadline timeIntervalSinceNow] > 0 && (self.probeDown-beforeDown < 2 || self.probeUp-beforeUp < 2)) {
        [NSRunLoop.currentRunLoop runMode:NSDefaultRunLoopMode beforeDate:[NSDate dateWithTimeIntervalSinceNow:0.01]];
    }
    NSUInteger downs = self.probeDown - beforeDown;
    NSUInteger ups = self.probeUp - beforeUp;
    return @{@"ok":@(downs == 2 && ups == 2), @"downs":@(downs), @"ups":@(ups), @"modifierOnly":@YES, @"doubleTapInputs":@(downs == 2)};
}
