...
- (void)build {
    ...
    self.stop=[NSButton buttonWithTitle:@"Stop" target:self action:@selector(stop:)]; self.stop.bezelStyle=NSBezelStyleInline; self.stop.refusesFirstResponder=YES; self.stop.frame=NSMakeRect(104,8,45,28); self.stop.accessibilityLabel=@"Stop recording"; [view addSubview:self.stop];
    self.cancel=[NSButton buttonWithTitle:@"×" target:self action:@selector(cancel:)]; self.cancel.bezelStyle=NSBezelStyleInline; self.cancel.refusesFirstResponder=YES; self.cancel.font=[NSFont systemFontOfSize:16]; self.cancel.frame=NSMakeRect(148,8,24,28); self.cancel.accessibilityLabel=@"Cancel dictation"; [view addSubview:self.cancel];
}
- (void)setPreferredScreenFrame:(NSValue *)screenFrame { self.preferredScreenFrame=screenFrame; }
- (void)position { if(self.panel.visible)return; NSRect v=self.preferredScreenFrame?self.preferredScreenFrame.rectValue:(NSScreen.mainScreen?:NSScreen.screens.firstObject).visibleFrame; NSRect f=self.panel.frame; f.origin=NSMakePoint(NSMidX(v)-NSWidth(f)/2,NSMinY(v)+36); [self.panel setFrame:f display:NO]; }
...
- (NSDictionary<NSString *,id> *)runInteractionProbe {
    ...
    return @{@"ok":@(ok),@"sourcePid":@(before),@"frontmostPid":@(NSWorkspace.sharedWorkspace.frontmostApplication.processIdentifier),@"fridayActive":@(NSApp.active),@"style":@"nonactivatingPanel",@"buttonRect":@{@"x":@(buttonRect.origin.x),@"y":@(buttonRect.origin.y),@"width":@(buttonRect.size.width),@"height":@(buttonRect.size.height)}};
}
@end
