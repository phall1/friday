...
const Model = struct {
    ...
    fn init(allocator: Allocator, spec: ModelSpec) !Model {
    ...
    fn clone(self: Model, allocator: Allocator) !Model {
        return init(allocator, self.toSpec());
    }

    fn toSpec(self: Model) ModelSpec {
        return .{
            .key = self.key,
            .id = self.id,
            .name = self.name,
    ...
    fn replace(allocator: Allocator, destination: *[]u8, value: []const u8) !void {
...
pub const ModelRepository = struct {
    ...
    pub fn activeModelKey(self: *const ModelRepository) u64 {
    ...
    pub fn activeModelPath(self: *const ModelRepository) ?[]const u8 {
        const io = self.state.threaded_io.io();
        self.state.snapshot_mutex.lockUncancelable(io);
        defer self.state.snapshot_mutex.unlock(io);
    ...
    fn copyForCommand(self: *ModelRepository, value: []const u8) ![]u8 {
        const io = self.state.threaded_io.io();
        self.state.mutex.lockUncancelable(io);
        defer self.state.mutex.unlock(io);
        if (self.state.stopping) return error.RepositoryClosed;
        return self.state.allocator.dupe(u8, value);
    }
...
const State = struct {
    ...
    fn executeResolveHF(self: *State, operation: IdentifierOperation) void {
        ...
        var model = self.modelFromHuggingFaceMetadata(metadata, identifier) catch |err| {
    ...
    fn persistResume(
        ...
        try json.endObject();
        try self.writeAtomic(resume_path, writer.written());
    }
    ...
    fn verifyAndPublish(
        ...
        self.writeAtomic(staged_manifest, initial_manifest) catch {
            self.fail(completion, "verified_manifest_write_failed", "The model manifest could not be written durably.");
            return;
        };
    ...
    fn saveIndex(self: *State) !void {
        ...
        try json.endArray();
        try json.endObject();
        try self.writeAtomic(self.index_path, writer.written());
    }
    ...
    fn writeAtomic(self: *State, path: []const u8, bytes: []const u8) !void {
        const temporary = try std.fmt.allocPrint(self.allocator, "{s}.{x}.tmp", .{ path, randomU64(self.threaded_io.io()) });
        defer self.allocator.free(temporary);
        var published = false;
        defer if (!published) Dir.cwd().deleteFile(self.threaded_io.io(), temporary) catch {};
        const file = try Dir.createFileAbsolute(self.threaded_io.io(), temporary, .{ .exclusive = true });
        errdefer file.close(self.threaded_io.io());
        try file.writeStreamingAll(self.threaded_io.io(), bytes);
        try file.sync(self.threaded_io.io());
        file.close(self.threaded_io.io());
        try Dir.renameAbsolute(temporary, path, self.threaded_io.io());
        published = true;
        try syncDirectory(self.threaded_io.io(), std.fs.path.dirname(path) orelse "/");
    }
    ...
    fn modelFromHuggingFaceMetadata(self: *State, bytes: []const u8, identifier: []const u8) !Model {
        ...
        defer language_slices.deinit(self.allocator);
        for (language_values[0..@min(language_values.len, 128)]) |language_value| switch (language_value) {
            .string => |language| if (language.len <= 32) language_slices.append(self.allocator, language) catch return error.OutOfMemory,
            else => {},
        };
        ...
        const encoded_artifact = try percentEncodeArtifact(self.allocator, artifact);
        defer self.allocator.free(encoded_artifact);
        const download_url = try std.fmt.allocPrint(self.allocator, "https://huggingface.co/{s}/resolve/{s}/{s}", .{ identifier, revision, encoded_artifact });
        defer self.allocator.free(download_url);
        return Model.init(self.allocator, .{
...
fn safeIdentifierPart(part: []const u8) bool {
...
fn modelKeyForIdentifier(identifier: []const u8) u64 {
...
fn sameIdentity(left: Model, right: Model) bool {
...
fn syncDirectory(io: Io, path: []const u8) !void {
...
fn hashFileHex(io: Io, path: []const u8) ![64]u8 {
    const file = try Dir.openFileAbsolute(io, path, .{});
    defer file.close(io);
...
fn runContractProbes() !struct {
    ...
    var candidate = try repository.state.modelFromHuggingFaceMetadata(valid_metadata, "fixture/model");
